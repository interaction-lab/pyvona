...
def create_voice(access_key, secret_key, voice="Brian"):
    """Creates and returns a voice object to interact with
    """
    return Voice(access_key, secret_key, voice)
...
class Voice(object):
    ...
    def fetch_marks(self, text_to_speak, filename):
        """Fetch a speechmarks file for given text and save it to the given file name
        """
        file_extension = ".{codec}".format(codec="txt")
        filename += file_extension if not filename.endswith(
            file_extension) else ""
        with open(filename, 'wb') as f:
            self.fetch_speechmarks_fp(text_to_speak, f)
    ...
    def fetch_speechmarks_fp(self, text_to_speak, fp):
        r = self._send_amazon_auth_packet_v4(
            'POST', 'tts', 'application/json', '/CreateSpeech', '',
            self._generate_speechmark_payload(text_to_speak), self._region, self._host)
    ...
    def fetch_speechmarks_str(self, text_to_speak):
        r = self._send_amazon_auth_packet_v4(
            'POST', 'tts', 'application/json', '/CreateSpeech', '',
            self._generate_speechmark_payload(text_to_speak), self._region, self._host)
    ...
    def speak(self, text_to_speak, interrupt = True, wait = False):
        """Speak a given text
        """
    ...
    def _generate_speechmark_payload(self, text_to_speak):
        return json.dumps({
            'Input': {
                'Data': text_to_speak
            },
    ...
    def __init__(self, access_key, secret_key, voice_name="Brian"):
